...
import ProfileLanguageSelect from "@/components/ProfileLanguageSelect";
...
export default function ProfileAvatarEditor({
  ...
  const saveAvatar = async (nextAvatarUrl: string | null) => {
    ...
    try {
      ...
      setSaveSuccess(t(locale, "settings.saved"));
      startTransition(() => {
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-6">
      <div className="space-y-5">
        <div>
          <h2 className="text-base font-semibold text-slate-900">{t(locale, "settings.personalTitle")}</h2>
          <p className="mt-1 text-sm text-slate-500">
            {t(locale, "settings.personalDescription")}
          </p>
        </div>

        <div className="flex items-start justify-between gap-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
          <div className="space-y-1">
            <span className="block text-sm font-medium text-slate-900">{t(locale, "settings.language")}</span>
            <span className="block text-xs leading-5 text-slate-500">
              {t(locale, "settings.languageDescription")}
            </span>
          </div>
          <ProfileLanguageSelect initialLocale={localePreference} />
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          <label className="space-y-1.5">
            <span className="text-sm font-medium text-slate-900">{t(locale, "settings.username")}</span>
            <input
              type="text"
              value={name}
              disabled
          ...
          <label className="space-y-1.5">
            <span className="text-sm font-medium text-slate-900">{t(locale, "settings.slug")}</span>
            <input
              type="text"
              value={draftSlug}
              onChange={(event) => {
          ...
          <label className="space-y-1.5">
            <span className="text-sm font-medium text-slate-900">{t(locale, "settings.email")}</span>
            <input
              type="email"
              value={email}
              disabled
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            ...
            <div className="min-w-0 flex-1 space-y-2">
              <div>
                <p className="text-sm font-medium text-slate-900">{t(locale, "settings.avatarTitle")}</p>
                <p className="text-xs text-slate-500">
                  {t(locale, "settings.avatarHint")}
                </p>
              </div>
              <input
                type="file"
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  {isSaving ? t(locale, "settings.saving") : t(locale, "settings.save")}
                </button>
                <button
                  type="button"
                  onClick={() => {
